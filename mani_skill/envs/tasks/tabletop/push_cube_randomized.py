...
# Support merging actors across subscenes
from mani_skill.utils.structs.actor import Actor

@register_env("PushCubeRandomized-v1", max_episode_steps=50)
class PushCubeRandomEnv(BaseEnv):
    """
    Task Description
    ----------------
    ...
    - TODO: table, ground, and cube support randomized colors and textures. Colors are any RGB value and textures are sampled from ~200 textures collected by https://github.com/robot-colosseum/robot-colosseum.
    - TODO: Ambient light supports RGB random value. Directed/pointed/spot lights can also be set with random pose.
    - TODO: Camera supports jittering from initial pose.
    - TODO: Distractor objects

    Success Conditions
    ------------------
    - the cube's xy position is within goal_radius (default 0.1) of the target's xy position by euclidean distance.

    Visualization: https://maniskill.readthedocs.io/en/latest/tasks/index.html#pushcube-v1 (unrandomized)
    TODO: Add visualization for randomized scene.
    """
    ...
    def _load_scene(self, options: dict):
        ...
        # texture_dir = "/robot-colosseum/colosseum/assets/textures"
        ...
        for i in range(self.num_envs):
            ...
            table = self.table[-1]
            path = random.sample(texture_files,1)[0]
            ...
            for part in obj._objs:
                for render_shape in part.find_component_by_type(sapien.render.RenderBodyComponent).render_shapes:
                    for triangle in render_shape.parts:
                        # Cycle through red, green, and blue cubes
                        color = np.array([0,0,0,255])
                        color[i%3] = 255
                        color = color / 255.0
                        triangle.material.set_base_color(color)
            """
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            print(f"Initializing episode: {env_idx=}")
            for i,table_scene in enumerate(self.table_scenes):
                # Most likely, robot is a batch-level object getting roped into individual subscene initializations

...
class PushCubeRandomizedEnv(PushCubeEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        # Loop over sub-scenes
        for i in range(self.num_envs):
            # Iterate through specified actors of the sub-scene
            for actor_name, rand_dict in options["actors"].items():
                ...
                for rand_type, rand_value in rand_dict.items():
                    if rand_type=="texture" and rand_value is True:
                        ...
                        self.randomize_color(obj=actor, color=rand_value)
    ...
    def _load_lighting(self, options: dict):
        if "lighting" not in options.keys():
            ...
            for i in range(len(self.sub_scenes)):
